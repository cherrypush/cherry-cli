...
class Codeowners {
  ...
  init() {
    ...
    if (codeownersDirectory.match(/\/(.github|.gitlab|docs)$/i))
      codeownersDirectory = path.dirname(codeownersDirectory);

    const codeownersFile = path.basename(codeownersFilePath);

    if (codeownersFile !== fileName)
      throw new Error(
        `Found a ${fileName} file but it was lower-cased: ${codeownersFilePath}`,
      );

    if (isDirectory(codeownersFilePath))
      throw new Error(
        `Found a ${fileName} but it's a directory: ${codeownersFilePath}`,
      );
  ...
  #globFiles(codeownersPath) {
    if (codeownersPath.includes("*"))
      return glob.sync(codeownersPath, { nodir: true });
