...
export const CONFIG_FILE_LOCAL_PATHS = [
  ".cherry.js",
  ".cherry.cjs",
  ".cherry.ts",
];
...
export const CONFIG_FILE_FULL_PATHS = CONFIG_FILE_LOCAL_PATHS.map(
  (filePath) => `${process.cwd()}/${filePath}`,
);
export const WORKFLOW_FILE_FULL_PATH = `${process.cwd()}/${WORKFLOW_FILE_LOCAL_PATH}`;

const CONFIG_TEMPLATE_PATH =
  dirname(fileURLToPath(import.meta.url)) + "/templates/.cherry.js.template";
const WORKFLOW_TEMPLATE_PATH =
  dirname(fileURLToPath(import.meta.url)) +
  "/templates/.cherry_push.yml.template";
...
export const createWorkflowFile = () => {
  ...
  fs.writeFileSync(
    WORKFLOW_FILE_FULL_PATH,
    fs.readFileSync(WORKFLOW_TEMPLATE_PATH).toString(),
  );
};

export const getConfigurationFile = () =>
  CONFIG_FILE_FULL_PATHS.find((filePath) => fs.existsSync(filePath));
