...
export const authorName = async (sha) =>
  (await git(`show ${sha} --format=%an --no-patch`))[0];

export const authorEmail = async (sha) =>
  (await git(`show ${sha} --format=%ae --no-patch`))[0];

export const commitDate = async (sha) =>
  new Date((await git(`show -s --format=%ci ${sha}`))[0]);
