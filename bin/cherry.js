...
program.command("init").action(async () => {
  ...
  console.log(
    "Your initial setup is done! Now try the command `cherry run` to see your first metrics.",
  );
});
...
program
  ...
  .option(
    "-f, --format <format>",
    "export format (json, sarif, sonar). default: json",
  )
  .action(async (options) => {
    ...
    const files = options.owner
      ? await getFiles(options.owner.split(","), codeOwners)
      : await getFiles();
    ...
    if (options.owner || options.metric) {
      ...
      if (owners)
        displayedOccurrences = displayedOccurrences.filter(
          (o) => _.intersection(o.owners, owners).length,
        );
      if (options.metric)
        displayedOccurrences = displayedOccurrences.filter(
          (o) => o.metricName === options.metric,
        );

      displayedOccurrences.forEach((occurrence) =>
        console.log(`👉 ${occurrence.text}`),
      );
...
program
  ...
  .action(async (options) => {
    ...
    if (!initialBranch)
      panic("Not on a branch, checkout a branch before pushing metrics.");
    const sha = await git.sha();

    const apiKey = options.apiKey || process.env.CHERRY_API_KEY;
    if (!apiKey)
      panic(
        "Please provide an API key with --api-key or CHERRY_API_KEY environment variable",
      );
...
program
  ...
  .option(
    "--error-if-increase",
    "Return an error status code (1) if the metric increased since its last report",
  )
  .action(async (options) => {
    const configuration = await getConfiguration();
    const apiKey = options.apiKey || process.env.CHERRY_API_KEY;
    ...
    for (const metric of metrics) {
      try {
        ...
        const response = await axios
          .get(API_BASE_URL + "/metrics", { params })
          .catch((error) => {
            console.error(
      ...
      if (diff > 0) {
        ...
        const newOccurrencesTexts = occurrences
          .filter((o) => o.metricName === metric)
          .map((o) => o.text);
        console.log(
          newOccurrencesTexts.filter((x) => !previousOccurrences.includes(x)),
        );
      }
...
program
  ...
  .option(
    "--since <since>",
    "yyyy-mm-dd | The date at which the backfill will start (defaults to 90 days ago)",
  )
  .option(
    "--until <until>",
    "yyyy-mm-dd | The date at which the backfill will stop (defaults to today)",
  )
  .option(
    "--interval <interval>",
    "The number of days between backfills (defaults to 30 days)",
  )
  .action(async (options) => {
    const since = options.since
      ? new Date(options.since)
      : substractDays(new Date(), 90);
    ...
    const initialBranch = await git.branchName();
    if (!initialBranch)
      panic("Not on a branch, checkout a branch before running the backfill.");
    const hasUncommitedChanges = (await git.uncommittedFiles()).length > 0;
    if (hasUncommitedChanges)
      panic("Please commit your changes before running this command");
    ...
    try {
      while (date >= since) {
        ...
        if (!sha) {
          console.log(
            `no commit found after ${toISODate(date)}, ending backfill`,
          );
          break;
        }
...
const upload = async (apiKey, projectName, date, occurrences) => {
  ...
  console.log("");
  console.log(
    `Uploading ${occurrences.length} occurrences in ${occurrencesBatches.length} batches:`,
  );
  ...
  _(occurrences)
    ...
    .mapValues((occurrences, metricName) => ({
      ...
      occurrences: occurrences.map((o) =>
        _.pick(o, "text", "value", "url", "owners"),
      ),
    }))
    .values()
    .flatten()
...
const uploadContributions = async (
  apiKey,
  projectName,
  authorName,
  authorEmail,
...
const countByMetric = (occurrences) =>
  _(occurrences)
    ...
    .mapValues((occurrences) =>
      _.sumBy(occurrences, (occurrence) =>
